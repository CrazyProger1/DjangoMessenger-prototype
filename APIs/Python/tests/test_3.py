...
class TestChat(unittest.TestCase):
    ...
    def test_chat_overflow(self):
        chat_id = self.user.create_chat('bla bla bla', False, False)
        self.user.add_chat_member(chat_id, self.user2._id)
        self.assertRaises(api.exceptions.ChatMemberError, lambda: self.user.add_chat_member(chat_id, self.user3._id))
